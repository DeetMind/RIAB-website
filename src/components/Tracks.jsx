const TRACKS = [
  ...
  },
  {
    ...
    outcomes: [
      'Asset pipeline ranked by avoided loss per public dollar — BCR calculated at 5% / 20-year for every project',
    ...
    person: {
      ...
      name: 'James McIntyre',
      role: 'Co-Founder',
      bio: 'Founder & Principal at Public Innovate; board member NYS HCR. 20-year public finance background (UBS, Morgan Stanley). Designed the financing logic behind RIAB\'s avoided-loss allocation.',
    },
  },
]
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-16">
          {TRACKS.map((t) => (
            <div key={t.id} className="rounded-2xl border overflow-hidden flex flex-col"
              ...
              {/* Tiers */}
              <div className="px-6 py-5 border-b" style={{ borderColor: t.accent + '20' }}>
                <p className="text-xs font-semibold uppercase tracking-wider text-muted mb-3">
                  How to engage
                </p>
              ...
              {/* Bio — inside card */}
              <div className="px-6 py-4 flex gap-3 items-start" style={{ background: t.accentLight + '60' }}>
                <div className="w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 text-white"
                  style={{ background: t.accent }}>
                  {t.person.initial}
                </div>
                <div>
                  <p className="font-semibold text-sm text-ink">{t.person.name}
                    <span className="font-normal text-xs text-muted ml-1.5">{t.person.role}</span>
                  </p>
                  <p className="text-xs text-muted leading-relaxed mt-0.5">{t.person.bio}</p>
                </div>
              </div>

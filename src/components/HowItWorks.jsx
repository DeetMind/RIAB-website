const steps = [
  {
    ...
    body: 'A standard intake template with one row per property or asset. Address, asset type, replacement value, current flood coverage. Takes about half a day to complete for 50 properties. We handle everything else.',
  ...
  },
  {
    ...
    body: 'Every property classified as loan-viable, grant-eligible, or blended, with the DSCR rationale your underwriting team can test. Interventions ranked by avoided loss per dollar. A two-page credit summary ready for your next committee meeting.',
      ...
      <div className="max-w-6xl mx-auto px-6">
            ...
            {steps.map((s) => (
              <div key={s.n} className="flex flex-col">
                ...
                <div className="bg-white rounded-2xl p-6 border border-border flex-1 flex flex-col">
                  ...
                  <h3 className="font-bold text-ink text-base mb-3 leading-snug">{s.title}</h3>
                  <p className="text-muted text-sm leading-relaxed mb-4 flex-1">{s.body}</p>
                  <p className="text-xs text-muted border-t border-border pt-3 italic">{s.detail}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mt-10 grid md:grid-cols-2 gap-4">
          <div className="bg-teal-light border-l-4 border-teal rounded-r-xl p-5">
            <p className="text-sm font-semibold text-teal-dark mb-1">One analysis, two uses</p>
          ...
          <div className="bg-white border border-border rounded-xl p-5">
            <p className="text-sm font-semibold text-ink mb-1">Pilots start with a free sample analysis</p>
